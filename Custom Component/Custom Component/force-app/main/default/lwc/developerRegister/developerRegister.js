...
import isEmailExist from '@salesforce/apex/DummyContactHandler.isEmailExist';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
export default class DeveloperRegister extends LightningElement {
    ...
    async handlSubmit() {
        ...
        if (this.fullName.phone.length === '') {
            this.check = true
            temp3.setCustomValidity("Complete this field.");
        } else if(this.fullName.phone.length !== 10){
            this.check = true
            temp3.setCustomValidity("Mobile number should be of 10 digits");
        }else if(!this.containsOnlyNumbers(this.fullName.phone)){
            this.check = true
            temp3.setCustomValidity("Mobile number only takes digits");
        }else{
        ...
        const isValid = emailPattern.test(this.fullName.email);
        console.log(' Is Email Valid ?  ', isValid);


        this.emailExist = false
        await isEmailExist({
            validEmail: this.fullName.email
        }).then(res => {
            this.emailExist = res
        }).catch(err => {
        ...
        console.log('Does Email exist ? : ', this.emailExist)

        let temp6 = this.template.querySelector(".c6");
        if (this.fullName.email === '') {
            this.check = true
            temp6.setCustomValidity('This is a Valid Field');
        } else if (!isValid) {
            this.check = true
            temp6.setCustomValidity("Enter a valid email");
        } else if (this.emailExist) {
            this.check = true
            temp6.setCustomValidity('Already an user exist with this email');
        } else {
        ...
        if(this.fullName.aadhar.length === 0){
            this.check = true;
            temp7.setCustomValidity('Complete this field')
        }else if(this.fullName.aadhar.length !== 16){
            this.check = true
            temp7.setCustomValidity('Adhaar number should be of 16 digits');
        }else if(!this.containsOnlyNumbers(this.fullName.aadhar)){
            this.check = true
            temp7.setCustomValidity('Adhaar number only takes digits');
        }else{
            temp7.setCustomValidity('Adhaar number only takes digits');
        }
